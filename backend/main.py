...
from core.exceptions import http_exception_handler, validation_exception_handler, api_exception_handler, general_exception_handler, APIException
...
app = FastAPI(title=settings.APP_NAME, version=settings.APP_VERSION, debug=settings.DEBUG, docs_url="/docs" if settings.DEBUG else None, redoc_url="/redoc" if settings.DEBUG else None)
...
async def secure_saas_api_boundary(request: Request, call_next):
    ...
    if secure_mode and not settings.ALLOW_LEGACY_API_IN_SAAS and path.startswith("/api/") and not path.startswith("/api/v1/"):
        return JSONResponse(status_code=410, content={"success": False, "detail": "This legacy endpoint is disabled in secure SaaS mode. Use the tenant-scoped /api/v1 API."})
    return await call_next(request)

@app.middleware("http")
async def protect_legacy_raw_pulse_endpoint(request: Request, call_next):
    if request.url.path == "/api/pulse/data":
        return JSONResponse(status_code=410, content={"success": False, "detail": "Bireysel pulse verisi gizlilik nedeniyle kapatıldı. /api/pulse/analytics kullanın."})
    return await call_next(request)
...
app.include_router(sso_v1.router)
...
async def health_check():
    ...
    return {"status": "healthy" if ready else "degraded", "ready": ready, "app_name": settings.APP_NAME, "version": settings.APP_VERSION, "environment": settings.ENVIRONMENT, "data_mode": settings.DATA_MODE, "secure_auth_enabled": settings.SAAS_AUTH_ENABLED, "database_configured": database_configured(), "database_ok": database_ok, "legacy_api_allowed": settings.ALLOW_LEGACY_API_IN_SAAS, "readiness_issue_count": len(readiness_issues)}

if __name__ == "__main__":
    import uvicorn
    uvicorn.run(app, host=settings.HOST, port=settings.PORT, reload=settings.RELOAD and settings.DEBUG, log_config=None)
