...
const LABEL_TO_CODE: Record<string, string> = {
  "Dijital Okuryazarlık": "DIG", "Analitik Düşünme": "ANA", "Sonuç Odaklılık": "RES", "Detaylara Özen": "DET", "Sürekli Öğrenme": "LRN", "Etik ve Uyum": "ETH", "Öz-Disiplin": "DIS", "Stratejik Bakış": "STR", "Takım Çalışması": "TEA", "İletişim Becerileri": "COM",
};
...
export default function YetenekMatrisiPage() {
  ...
  const [ai, setAi] = useState<{mode:"ai"|"rules";recommendation:string;note?:string}|null>(null);
  ...
  const gapData = Object.entries(target).map(([label, expected]) => {
    ...
    return { label, actual, expected:Number(expected), gap };
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between"><div><p className="text-xs font-semibold uppercase tracking-[0.12em] text-indigo-600">Yetenek karar desteği</p><h1 className="mt-1 text-2xl font-semibold text-slate-900 dark:text-white">Yetenek Matrisi</h1><p className="mt-1 max-w-3xl text-sm text-slate-500">Potansiyel artık performansın kopyası değil: öğrenme çevikliği %30, liderlik kapasitesi %25, uyum %15, karmaşıklık yönetimi %10, kariyer isteği %10 ve mobilite isteği %10.</p></div><div className="flex gap-2"><Link href="/kariyer" className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-semibold">Kariyer mimarisi</Link><Link href="/yedekleme" className="rounded-xl bg-indigo-600 px-3 py-2 text-xs font-semibold text-white">Halefiyet</Link></div></div>
