...
export default function YoneticiMaasTalepPage(){
  ...
  return <div className="space-y-5">
    ...
    <div className="overflow-hidden rounded-xl border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900"><div className="overflow-x-auto"><table className="w-full"><thead><tr><th className="!text-slate-900 dark:!text-slate-100">Çalışan</th><th className="text-right !text-slate-900 dark:!text-slate-100">Mevcut Maaş</th><th className="text-right !text-slate-900 dark:!text-slate-100">Sistem Referansı</th><th className="!text-slate-900 dark:!text-slate-100">Yönetici Talebi %</th><th className="!text-slate-900 dark:!text-slate-100">Gerekçe</th></tr></thead><tbody>{rows.length?rows.map(row=><tr key={row.id||row.name}><td>{row.name}</td><td className="text-right font-mono">{row.current.toLocaleString("tr-TR")}</td><td className="text-right font-mono">{Math.round(row.baseline).toLocaleString("tr-TR")}</td><td><input disabled={!inputOpen} type="number" min="0" max="100" step="0.5" value={row.proposal.rate??""} onChange={e=>setProposals({...proposals,[row.name]:{...row.proposal,rate:e.target.value===""?null:Number(e.target.value)}})} className="w-24 rounded-lg border border-slate-200 p-2 text-sm disabled:bg-slate-100"/></td><td><input disabled={!inputOpen} value={row.proposal.note} onChange={e=>setProposals({...proposals,[row.name]:{...row.proposal,note:e.target.value}})} placeholder="Gerekçe" className="min-w-[240px] rounded-lg border border-slate-200 p-2 text-sm disabled:bg-slate-100"/></td></tr>):<tr><td colSpan={5} className="py-8 text-center text-sm text-slate-500">Yönetilebilir çalışan bulunmuyor.</td></tr>}</tbody></table></div></div>
...
function Mini({label,value}:{label:string;value:string}){return <div className="enterprise-card p-4"><p className="text-[10px] uppercase tracking-wide text-slate-600">{label}</p><p className="mt-2 text-sm font-semibold">{value}</p></div>}
