...
import {
  calculateCompetencyScore, calculatePerformance, DEFAULT_KPIS, PERFORMANCE_MODEL_VERSION, type KpiItem,
...
export default function DegerlendirmePage() {
  ...
  useEffect(() => {
    ...
    if (!latest) {
      setScores({ ...emptyScores }); setKpis(cloneDefaultKpis()); setManagerPerformance(3); setNote(""); setIsStarPerformer(false); return;
    }
    ...
    setKpis(storedKpis.map((item: any, index: number) => ({
      id: String(item.id || `kpi-${index + 1}`), title: String(item.title || `Hedef ${index + 1}`),
      weight: Number(item.weight || 0), score: Number(item.score || legacyPerformance),
    })));
  ...
  const addKpi = () => setKpis((items) => [...items, { id: `kpi-${Date.now()}`, title: `Yeni hedef ${items.length + 1}`, weight: 0, score: 3 }]);
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="mt-1 max-w-4xl text-sm text-slate-500">Nihai performans = KPI/Hedef Başarısı %60 + Yönetici Performans Gözlemi %40. Yetkinlik ayrı ölçülür. Değerlendirme yetkisi pozisyondan değil Yönetici 1 / Yönetici 2 ilişkisinden gelir.</p>
      </div>
        ...
        <div className="space-y-5">
          <fieldset disabled={!canEdit} className={!canEdit ? "opacity-70" : ""}>
            <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
              ...
              <div className="mt-5 border-t border-slate-100 pt-4"><div className="flex justify-between text-xs"><span className="font-medium text-slate-600">Yönetici performans gözlemi · %40</span><strong className="text-blue-700">{managerPerformance.toFixed(1)} / 5</strong></div><input type="range" min="1" max="5" step="0.1" value={managerPerformance} onChange={(e)=>setManagerPerformance(Number(e.target.value))} className="mt-2 w-full"/></div>
            </section>

            <section className="mt-5 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900"><div className="flex items-center gap-2"><Target className="h-4 w-4 text-violet-600"/><h2 className="text-sm font-semibold">Yetkinlik puanlama</h2></div><p className="mt-1 text-xs text-slate-500">Yetkinlik ayrı bir skor üretir ve potansiyel hesabının girdilerinden biridir.</p><div className="mt-4 grid gap-x-6 gap-y-4 lg:grid-cols-2">{Object.entries(COMPETENCIES).map(([code,label]) => <div key={code}><div className="flex justify-between text-xs"><span>{label}</span><strong className="text-violet-700">{scores[code].toFixed(1)}</strong></div><input type="range" min="1" max="5" step="0.1" value={scores[code]} onChange={(e)=>setScores({...scores,[code]:Number(e.target.value)})} className="mt-2 w-full"/></div>)}</div><label className="mt-5 block text-xs font-medium text-slate-600">Yönetici notu {access.override && <span className="text-red-500">· override gerekçesi</span>}<textarea value={note} onChange={(e)=>setNote(e.target.value)} rows={3} className="mt-1 w-full rounded-xl border border-slate-200 p-3 text-sm"/></label><button type="button" onClick={save} className="mt-4 inline-flex items-center gap-2 rounded-xl bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white"><Save className="h-4 w-4"/>Değerlendirmeyi kaydet</button></section>
          </fieldset>
        </div>
      </div>
...
function MiniScore({label,value}:{label:string;value:number}){return <div className="px-2"><p className="text-[9px] font-bold uppercase tracking-wide text-slate-400">{label}</p><p className="mt-1 text-lg font-semibold text-slate-900 dark:text-white">{value.toFixed(2)}</p></div>}
