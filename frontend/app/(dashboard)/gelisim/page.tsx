...
import {
  createSaasDevelopmentPlan,
  fetchSaasDevelopmentWorkspace,
  SAAS_DATA_MODE,
  transferSaasDevelopmentPlan,
  updateSaasDevelopmentPlanStatus,
...
export default function GelisimPage() {
  ...
  useEffect(() => {
    const reload = async () => {
      ...
      try {
        if (SAAS_DATA_MODE) {
          ...
          setPlans(workspace.plans as DevelopmentPlan[]);
          setAssessments(workspace.evaluations);
        ...
        setLoadError(error instanceof Error ? error.message : "Gelişim verisi yüklenemedi.");
        setOrgData([]); setPlans([]); setAssessments([]); setAssignments([]);
      }
    };
    void reload();
    ...
    return () => { window.removeEventListener("dataUpdated", refresh); window.removeEventListener("userChanged", refresh); };
  }, []);

  const role = String(user?.role || "").toUpperCase();
  const isEmployee = role === "PERSONEL" || role === "EMPLOYEE";
  const canCreatePlan = ["CEO", "IK", "HR_ADMIN", "DIRECTOR", "MANAGER"].includes(role);
  ...
  const savePlans = (next: DevelopmentPlan[]) => {
    setPlans(next);
    if (!SAAS_DATA_MODE) setStorageData(STORAGE_KEYS.DEVELOPMENT_PLANS, next);
  ...
  const create = async (event: FormEvent) => {
    ...
    try {
      ...
      if (SAAS_DATA_MODE) {
        const employee = orgData.find((row) => row["Ad Soyad"] === selectedName);
        if (!employee?.id) return showToast("Çalışanın SaaS kimliği bulunamadı.", "error");
        plan = await createSaasDevelopmentPlan({
      ...
      showToast(error instanceof Error ? error.message : "Gelişim aksiyonu kaydedilemedi.", "error");
    }
  };

  const updateStatus = async (id: string, status: DevelopmentPlan["status"]) => {
    try {
      if (SAAS_DATA_MODE) {
        const updated = await updateSaasDevelopmentPlanStatus(id, status);
        savePlans(plans.map((plan) => plan.id === id ? updated as DevelopmentPlan : plan));
        return;
      }
      savePlans(plans.map((plan) => plan.id === id ? { ...plan, status } : plan));
    } catch (error) {
      showToast(error instanceof Error ? error.message : "Gelişim durumu güncellenemedi.", "error");
    }
  };

  const sendToTraining = async (plan: DevelopmentPlan) => {
    if (!canCreatePlan) return;
    try {
      if (SAAS_DATA_MODE) {
        if (assignments.some((assignment) => assignment.sourceDevelopmentPlanId === plan.id)) return showToast("Bu aksiyon zaten Eğitim modülüne aktarıldı.", "info");
        const assignment = await transferSaasDevelopmentPlan(plan.id);
        setAssignments([assignment, ...assignments.filter((item) => item.id !== assignment.id)]);
      ...
      showToast(error instanceof Error ? error.message : "Eğitim aktarımı tamamlanamadı.", "error");
    }
  };

  if (!employees.length) return <div className="enterprise-card p-8 text-center"><UserRound className="mx-auto h-8 w-8 text-slate-300"/><h1 className="mt-3 text-lg font-semibold">Gelişim profili bulunamadı</h1><p className="mt-1 text-sm text-slate-500">{loadError || (SAAS_DATA_MODE ? "Yetkili gelişim kapsamınızda çalışan bulunmuyor." : "Demo personanız organizasyon kaydıyla eşleşmiyor.")}</p></div>;
  ...
  return <div className="space-y-5">
    ...
    {SAAS_DATA_MODE&&<div className="rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-xs leading-5 text-emerald-800">Gelişim planları ve Eğitim & Gelişim aktarımları tenant-scoped SaaS veritabanında tutuluyor; production modunda browser storage fallback kullanılmıyor.</div>}
