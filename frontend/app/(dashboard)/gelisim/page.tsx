...
import { ArrowRight, BookOpen, Plus, Sparkles, Target, UserRound } from "lucide-react";
...
export default function GelisimPage() {
  ...
  if (!employees.length) return <div className="rounded-2xl border border-slate-200 bg-white p-8 text-center shadow-sm"><UserRound className="mx-auto h-8 w-8 text-slate-300"/><h1 className="mt-3 text-lg font-semibold">Gelişim profili bulunamadı</h1><p className="mt-1 text-sm text-slate-500">Demo personanız organizasyon kaydıyla eşleşmiyor.</p></div>;

  return <div className="space-y-5">
    <div><p className="text-xs font-semibold uppercase tracking-[.12em] text-emerald-600">{isEmployee ? "Benim gelişimim" : "Gelişim planlama"}</p><h1 className="mt-1 text-2xl font-semibold">Gelişim Planı</h1><p className="mt-1 text-sm text-slate-500">Yetkinlik açığı → FutureHR Gelişim Reçetesi → uygulama → başarı kanıtı → yeniden ölçüm zinciri.</p></div>

    <div className="grid gap-5 xl:grid-cols-[360px_1fr]"><div className="space-y-4"><div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">{isEmployee ? <div><p className="text-[10px] font-semibold uppercase tracking-[.1em] text-slate-400">Çalışan</p><p className="mt-1 text-sm font-semibold">{selectedName}</p></div> : <label className="text-xs font-medium text-slate-500">Çalışan<select value={selectedName} onChange={(e) => setSelectedName(e.target.value)} className="mt-1 w-full rounded-xl border border-slate-200 p-2.5 text-sm">{employees.map((employee: any) => <option key={employee.id ?? employee["Ad Soyad"]}>{employee["Ad Soyad"]}</option>)}</select></label>}<div className="mt-4 rounded-xl bg-emerald-50 p-3"><p className="text-xs font-semibold text-emerald-900">{selectedOrg.Pozisyon || "Pozisyon"}</p><p className="mt-1 text-xs text-emerald-700">{gaps.length} yetkinlik açığı · {learningEvidence.length} tamamlanmış gelişim kanıtı</p></div></div><div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-center gap-2"><Target className="h-4 w-4 text-emerald-600"/><h2 className="text-sm font-semibold">Öncelikli gelişim alanları</h2></div><div className="mt-3 space-y-2">{gaps.slice(0,5).map((gap) => <div key={gap.label} className="rounded-xl bg-slate-50 p-3 text-xs"><div className="flex justify-between"><span className="font-medium">{gap.label}</span><span className="font-semibold text-red-600">-{gap.gap.toFixed(1)}</span></div><p className="mt-1 text-slate-400">{gap.actual.toFixed(1)} → hedef {gap.expected.toFixed(1)}</p></div>)}{!gaps.length && <p className="text-xs text-slate-500">Ölçülebilir açık bulunmuyor.</p>}</div></div></div>

      <div className="space-y-4"><div className="rounded-2xl border border-indigo-200 bg-indigo-50/60 p-5 dark:border-indigo-900/50 dark:bg-indigo-950/20"><div className="flex items-center gap-2"><Sparkles className="h-4 w-4 text-indigo-600"/><h2 className="text-sm font-semibold text-indigo-950 dark:text-indigo-200">FutureHR Gelişim Reçetesi</h2></div><p className="mt-1 text-[11px] text-indigo-700/80">En büyük yetkinlik açıkları için kütüphaneden seviyelendirilmiş müdahaleler. Öneri otomatik karar değildir.</p><div className="mt-3 grid gap-2 md:grid-cols-2">{prescriptions.map((item) => <button key={item.id} type="button" onClick={() => canCreatePlan && usePrescription(item)} className="rounded-xl border border-indigo-100 bg-white p-3 text-left hover:border-indigo-300 dark:bg-slate-900"><div className="flex items-center justify-between"><span className="text-[9px] font-bold uppercase text-indigo-600">{item.competencyCode} · L{item.level} · {item.type}</span><span className="text-[10px] font-semibold text-red-600">-{item.gap.toFixed(1)}</span></div><p className="mt-1 text-xs font-semibold">{item.name}</p><p className="mt-1 text-[10px] text-slate-500">{item.duration} · yeniden ölçüm {item.reassessDays} gün</p></button>)}</div><Link href="/egitim" className="mt-3 inline-flex items-center gap-1 text-[11px] font-semibold text-indigo-700">60 müdahalelik kütüphaneyi aç <ArrowRight className="h-3.5 w-3.5"/></Link></div>

        {canCreatePlan && <form onSubmit={create} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-center gap-2"><Plus className="h-4 w-4 text-emerald-600"/><h2 className="text-sm font-semibold">Gelişim aksiyonu oluştur</h2>{form.interventionId && <span className="rounded-full bg-indigo-50 px-2 py-1 text-[9px] font-bold text-indigo-700">Kütüphaneden</span>}</div><div className="mt-4 grid gap-3 sm:grid-cols-2"><Field label="Yetkinlik / alan"><input value={form.competency} onChange={(e) => setForm({ ...form, competency: e.target.value })} className="w-full rounded-xl border border-slate-200 p-2.5 text-sm"/></Field><Field label="Aksiyon tipi"><select value={form.actionType} onChange={(e) => setForm({ ...form, actionType: e.target.value as DevelopmentPlan["actionType"] })} className="w-full rounded-xl border border-slate-200 p-2.5 text-sm">{ACTION_TYPES.map((type) => <option key={type}>{type}</option>)}</select></Field><Field label="Gelişim hedefi"><textarea value={form.goal} onChange={(e) => setForm({ ...form, goal: e.target.value })} rows={3} className="w-full rounded-xl border border-slate-200 p-2.5 text-sm"/></Field><Field label="Aksiyon"><textarea value={form.action} onChange={(e) => setForm({ ...form, action: e.target.value })} rows={3} className="w-full rounded-xl border border-slate-200 p-2.5 text-sm"/></Field><Field label="Başarı ölçütü"><input value={form.successMetric} onChange={(e) => setForm({ ...form, successMetric: e.target.value })} className="w-full rounded-xl border border-slate-200 p-2.5 text-sm"/></Field><Field label="Son tarih"><input type="date" value={form.dueDate} onChange={(e) => setForm({ ...form, dueDate: e.target.value })} className="w-full rounded-xl border border-slate-200 p-2.5 text-sm"/></Field></div><button className="mt-4 rounded-xl bg-emerald-600 px-4 py-2.5 text-sm font-semibold text-white">Aksiyonu ekle</button></form>}
      </div></div>

    {selectedName && <AIDecisionSupport kind="development" context={aiContext} resetKey={selectedName} title={isEmployee ? "AI Gelişim Rehberi" : "AI Gelişim Karar Desteği"} description="Yetkinlik açığı, rol hedefi, aktif planlar, tamamlanan öğrenme kanıtları ve FutureHR Gelişim Reçetesini birlikte yorumlar." buttonLabel="Gelişim analizini oluştur" questionTitle="Gelişim görüşmesi soruları"/>}

    <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-center justify-between"><div><h2 className="text-sm font-semibold">Aktif gelişim planı</h2><p className="mt-1 text-xs text-slate-500">Kütüphane müdahaleleri Eğitim & Gelişim modülüne aktarılıp tamamlandığında Evidence Graph'a gelişim kanıtı olarak taşınır.</p></div><Link href="/egitim" className="inline-flex items-center gap-1 text-xs font-semibold text-emerald-700"><BookOpen className="h-3.5 w-3.5"/> Eğitim & Gelişim <ArrowRight className="h-3.5 w-3.5"/></Link></div><div className="mt-4 space-y-3">{employeePlans.length ? employeePlans.map((plan) => <div key={plan.id} className="rounded-2xl border border-slate-100 p-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><div className="flex flex-wrap items-center gap-2"><span className="rounded-full bg-emerald-50 px-2 py-1 text-[10px] font-semibold text-emerald-700">{plan.actionType}</span>{plan.competency && <span className="text-xs text-slate-400">{plan.competency}</span>}{plan.interventionId && <span className="rounded-full bg-indigo-50 px-2 py-1 text-[9px] font-bold text-indigo-700">FutureHR Reçetesi</span>}</div><h3 className="mt-2 text-sm font-semibold">{plan.goal}</h3><p className="mt-1 text-xs text-slate-600">Aksiyon: {plan.action}</p><p className="mt-1 text-xs text-slate-400">Ölçüt: {plan.successMetric}{plan.reassessDays ? ` · yeniden ölçüm ${plan.reassessDays} gün` : ""}</p></div>{canCreatePlan ? <select value={plan.status} onChange={(e) => updateStatus(plan.id, e.target.value as DevelopmentPlan["status"])} className="rounded-lg border border-slate-200 px-2 py-1.5 text-xs"><option>Planlandı</option><option>Devam Ediyor</option><option>Tamamlandı</option></select> : <span className="rounded-full bg-slate-100 px-2 py-1 text-[10px] font-semibold text-slate-600">{plan.status}</span>}</div>{canCreatePlan && <button onClick={() => sendToTraining(plan)} disabled={plan.transferredToTraining} className="mt-3 rounded-lg bg-violet-50 px-3 py-2 text-xs font-semibold text-violet-700 disabled:opacity-50">{plan.transferredToTraining ? "Eğitim & Gelişime aktarıldı" : "Eğitim & Gelişim modülüne aktar"}</button>}</div>) : <p className="py-8 text-center text-sm text-slate-500">Henüz gelişim aksiyonu yok.</p>}</div></div>
  </div>;
}
