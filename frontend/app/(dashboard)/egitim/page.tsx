...
import { BookOpen, CheckCircle2, Clock3, Filter, Plus, Search, Sparkles, Users } from "lucide-react";
...
const levelOptions: Array<{ value: "" | DevelopmentLevel; label: string }> = [
  { value: "", label: "Tüm seviyeler" }, { value: 1, label: "1 · Temel" }, { value: 2, label: "2 · Uygulama" }, { value: 3, label: "3 · İleri" }, { value: 4, label: "4 · Liderlik" },
];
...
export default function EgitimPage() {
  ...
  useEffect(() => { reload(); const handler = () => reload(); window.addEventListener("dataUpdated", handler); return () => window.removeEventListener("dataUpdated", handler); }, []);
  ...
  const teamAssignments = assignments.filter((assignment) => manageable.some((employee: any) => employee["Ad Soyad"] === assignment.employee));
  ...
  const assign = (event: FormEvent) => {
    ...
    const item: TrainingAssignment = {
      id: `training-${Date.now()}`, employee: form.employee, trainingId: intervention.id, trainingName: intervention.name,
      assignedBy: name, assignedAt: new Date().toISOString(), dueDate: form.dueDate || undefined, status: "Atandı",
      competencyCode: intervention.competencyCode, developmentLevel: intervention.level, interventionType: intervention.type,
      transferTask: intervention.transferTask, successMetric: intervention.successMetric,
    };
  ...
  const setStatus = (id: string, status: TrainingAssignment["status"]) => {
    const next = assignments.map((assignment) => {
      ...
      return {
        ...
        reassessDueAt: completed ? datePlusDays(intervention?.reassessDays || 60) : undefined,
      };
    });
    saveAssignments(next);
    showToast(status === "Tamamlandı" ? "Tamamlandı. Gelişim kanıtı Evidence Graph için kaydedildi ve yeniden ölçüm tarihi oluşturuldu." : "Eğitim durumu güncellendi.", "success");
  };
  ...
  const tabItems = [
    ...
    ...(manageable.length ? [{ key: "manage" as const, label: "Ekip Gelişimi", description: "Yetkinlik açığına göre müdahale atayın ve ilerlemeyi yönetin.", icon: Users, count: teamAssignments.length }] : []),
  ];

  return <div className="space-y-5">
    <div><p className="text-xs font-semibold uppercase tracking-[0.12em] text-violet-600">Kanıta dayalı gelişim sistemi</p><h1 className="mt-1 text-2xl font-semibold text-slate-900 dark:text-white">Eğitim & Gelişim Kütüphanesi</h1><p className="mt-1 max-w-5xl text-sm text-slate-500">FutureHR yalnız “kurs tamamlandı” takibi yapmaz. Yetkinlik açığını seviyelendirilmiş öğrenme, iş üstü uygulama, geri bildirim, transfer görevi ve 30-90 günlük yeniden ölçüm döngüsüne bağlar.</p></div>

    <div className="grid gap-3 sm:grid-cols-3"><Metric label="Bana atanan" value={myAssignments.length} icon={BookOpen}/><Metric label="Devam eden" value={myAssignments.filter((item) => item.status !== "Tamamlandı").length} icon={Clock3}/><Metric label="Tamamlanan / kanıt" value={myAssignments.filter((item) => item.status === "Tamamlandı").length} icon={CheckCircle2}/></div>
    ...
    <div className="rounded-2xl border border-violet-200/80 bg-white p-2 shadow-[0_8px_30px_rgba(76,29,149,0.08)] dark:border-violet-900/50 dark:bg-slate-900">
      <div className="grid gap-2 md:grid-cols-3">{tabItems.map(({ key, label, description, icon: Icon, count }) => { const active = tab === key; return <button key={key} type="button" onClick={() => setTab(key)} className={`flex min-h-[76px] items-center gap-3 rounded-xl border px-4 py-3 text-left ${active ? "border-violet-600 bg-violet-600 text-white shadow-lg" : "border-slate-200 bg-slate-50/70 text-slate-700 hover:border-violet-300 dark:border-slate-700 dark:bg-slate-800/60 dark:text-slate-200"}`}><span className={`flex h-10 w-10 items-center justify-center rounded-xl ${active ? "bg-white/15" : "bg-white text-violet-600 dark:bg-slate-900"}`}><Icon className="h-5 w-5"/></span><span className="min-w-0 flex-1"><span className="flex items-center gap-2"><strong className="text-sm">{label}</strong><span className={`rounded-full px-2 py-0.5 text-[10px] font-bold ${active ? "bg-white/15" : "bg-violet-100 text-violet-700"}`}>{count}</span></span><span className={`mt-1 block text-[11px] leading-4 ${active ? "text-violet-100" : "text-slate-500"}`}>{description}</span></span></button>; })}</div>
    </div>

    {tab === "mine" && <PremiumTrainingTable title="Atanan Gelişim Müdahaleleri" description="Tamamlanan müdahaleler yeniden ölçüm tarihiyle gelişim kanıtına dönüşür." rows={myAssignments} editable onStatus={setStatus} overdue={overdue}/>} 

    {tab === "catalog" && <>
      <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center"><div className="relative flex-1"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400"/><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Eğitim, yetkinlik veya yöntem ara..." className="h-10 w-full rounded-xl border border-slate-200 pl-9 pr-3 text-sm"/></div><div className="flex gap-2"><select value={competencyFilter} onChange={(e) => setCompetencyFilter(e.target.value)} className="h-10 rounded-xl border border-slate-200 px-3 text-sm"><option value="">10 yetkinliğin tümü</option>{Array.from(new Map(DEVELOPMENT_LIBRARY.map((item) => [item.competencyCode, item.competencyLabel])).entries()).map(([code, label]) => <option key={code} value={code}>{label}</option>)}</select><select value={levelFilter} onChange={(e) => setLevelFilter(e.target.value ? Number(e.target.value) as DevelopmentLevel : "")} className="h-10 rounded-xl border border-slate-200 px-3 text-sm">{levelOptions.map((option) => <option key={String(option.value)} value={option.value}>{option.label}</option>)}</select></div></div>
      ...
      {recommendations.length > 0 && <div className="rounded-2xl border border-indigo-200 bg-indigo-50/60 p-4 dark:border-indigo-900/50 dark:bg-indigo-950/20"><div className="flex items-center gap-2"><Sparkles className="h-4 w-4 text-indigo-600"/><h2 className="text-sm font-semibold text-indigo-950 dark:text-indigo-200">Kişiye göre önerilen gelişim reçetesi</h2></div><p className="mt-1 text-[11px] text-indigo-700/80 dark:text-indigo-300">Mevcut rol hedefi ile son yetkinlik kanıtı arasındaki en büyük açıklardan seçildi. Otomatik atama değildir.</p><div className="mt-3 grid gap-2 md:grid-cols-2 xl:grid-cols-3">{recommendations.map((item) => <button key={item.id} type="button" onClick={() => { setTab(manageable.length ? "manage" : "catalog"); setForm((value) => ({ ...value, trainingId: item.id })); }} className="rounded-xl border border-indigo-100 bg-white p-3 text-left hover:border-indigo-300 dark:bg-slate-900"><div className="flex items-center justify-between"><span className="text-[9px] font-bold uppercase text-indigo-600">{item.competencyLabel} · L{item.level}</span><span className="text-[10px] font-semibold text-red-600">gap -{item.gap.toFixed(1)}</span></div><p className="mt-1 text-xs font-semibold">{item.name}</p><p className="mt-1 text-[10px] text-slate-500">{item.type} · {item.duration}</p></button>)}</div></div>}
    ...
    {tab === "manage" && <div className="grid items-start gap-5 xl:grid-cols-[380px_1fr]"><form onSubmit={assign} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900"><div className="flex items-center gap-2"><Plus className="h-4 w-4 text-violet-600"/><h2 className="text-sm font-semibold">Gelişim müdahalesi ata</h2></div><div className="mt-4 space-y-3"><select value={form.employee} onChange={(e) => setForm({ ...form, employee: e.target.value })} className="w-full rounded-xl border border-slate-200 p-2.5 text-sm"><option value="">Çalışan seçin</option>{manageable.map((employee: any) => <option key={employee.id ?? employee["Ad Soyad"]}>{employee["Ad Soyad"]}</option>)}</select>{recommendations.length > 0 && <div className="rounded-xl bg-indigo-50 p-3 text-[10px] leading-4 text-indigo-800"><strong>FutureHR reçetesi:</strong> Seçili çalışan için {recommendations.length} uygun müdahale önerildi. Aşağıdaki listede ★ ile gösterilir.</div>}<select value={form.trainingId} onChange={(e) => setForm({ ...form, trainingId: e.target.value })} className="w-full rounded-xl border border-slate-200 p-2.5 text-sm"><option value="">Müdahale seçin</option>{DEVELOPMENT_LIBRARY.map((item) => <option key={item.id} value={item.id}>{recommendedIds.has(item.id) ? "★ " : ""}{item.competencyCode} · L{item.level} · {item.name}</option>)}</select>{form.trainingId && (() => { const item = findDevelopmentIntervention(form.trainingId); return item ? <div className="rounded-xl border border-slate-200 p-3 text-[10px] leading-4 text-slate-500"><p><strong className="text-slate-700">Transfer görevi:</strong> {item.transferTask}</p><p className="mt-1"><strong className="text-slate-700">Başarı ölçütü:</strong> {item.successMetric}</p><p className="mt-1"><strong className="text-slate-700">Yeniden ölçüm:</strong> tamamlandıktan {item.reassessDays} gün sonra</p></div> : null; })()}<label className="block text-xs font-medium text-slate-600">Son tarih<input type="date" value={form.dueDate} onChange={(e) => setForm({ ...form, dueDate: e.target.value })} className="mt-1 w-full rounded-xl border border-slate-200 p-2.5 text-sm"/></label><button className="w-full rounded-xl bg-violet-600 px-4 py-2.5 text-sm font-semibold text-white">Atama Yap</button></div></form><PremiumTrainingTable title="Ekip Gelişim Takibi" description="Atanmış gelişim müdahalelerini, son tarihleri ve Evidence Graph'a dönüşecek tamamlanma kanıtlarını izleyin." rows={teamAssignments} overdue={overdue}/></div>}
  </div>;
}
