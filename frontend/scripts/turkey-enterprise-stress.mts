...
import {
  ...
  detectImportDataset,
  parseTurkishNumber,
} from "../lib/hr/dataImport.ts";
import { normalizeEmployeeMaster, readinessChecklist } from "../lib/hr/turkeyEnterprise.ts";

type Result = { name: string; pass: boolean; actual?: unknown; expected?: unknown };
const results: Result[] = [];
const add = (name: string, pass: boolean, actual?: unknown, expected?: unknown) => results.push({ name, pass, actual, expected });
...
add("republic_eve_half_day", halfHoliday.days === 0.5, halfHoliday.days, 0.5);
const fullHoliday = calculateLeaveDays("2026-10-29", "2026-10-29", "annual");
add("republic_day_not_charged", fullHoliday.days === 0, fullHoliday.days, 0);
const parsedMoney = parseTurkishNumber("75.000,50 TL");
add("turkish_money_parser", parsedMoney === 75000.5, parsedMoney, 75000.5);
...
add("canonical_payroll_mapping", payroll?.employeeCode === "P001" && payroll?.period === "2026-08" && payroll?.grossSalary === 75000.5 && payroll?.netSalary === 56000.25, payroll, "P001/2026-08/75000.5/56000.25");
...
add("canonical_attendance_mapping", attendance?.employeeCode === "P001" && attendance?.workedMinutes === 480 && attendance?.overtimeMinutes === 30 && attendance?.firstIn === "08:30", attendance, "P001/480/30/08:30");
...
add("canonical_employee_master", master?.employeeCode === "P001" && master?.department === "Finans" && master?.position === "Finans Uzmanı" && master?.costCenter === "CC-300", master, "canonical employee fields");
...
for (const result of results) {
  console.log(`${result.pass ? "PASS" : "FAIL"} | ${result.name}${result.pass ? "" : ` | actual=${JSON.stringify(result.actual)} expected=${JSON.stringify(result.expected)}`}`);
}
