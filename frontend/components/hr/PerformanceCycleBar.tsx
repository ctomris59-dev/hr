...
import { CheckCircle2, LockKeyhole, PlayCircle, Scale, Target } from "lucide-react";
...
export default function PerformanceCycleBar() {
  ...
  const Icon = icon;
  return (
    <section className="mb-4 overflow-hidden rounded-lg border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900">
      <div className="flex flex-col gap-3 px-4 py-3 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex min-w-0 items-start gap-3">
          <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-[#2f6664] text-white"><Icon className="h-4 w-4"/></span>
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2"><p className="text-xs font-semibold text-slate-900 dark:text-white">{cycle.name}</p><span className="rounded-md border border-slate-200 px-2 py-0.5 text-[9px] font-semibold text-slate-600 dark:border-slate-700 dark:text-slate-300">{PERFORMANCE_STAGE_LABELS[cycle.stage]}</span></div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="min-w-[170px]"><div className="mb-1 flex items-center justify-between text-[9px] text-slate-400"><span>Dönem kapsamı</span><b className="text-slate-600 dark:text-slate-300">%{completion.rate}</b></div><div className="h-1.5 overflow-hidden rounded-full bg-slate-200 dark:bg-slate-800"><div className="h-full rounded-full bg-[#2f6664]" style={{ width: `${completion.rate}%` }}/></div></div>
          {canManage && cycle.stage !== "LOCKED" && <button type="button" onClick={advance} className="inline-flex h-8 items-center gap-1.5 rounded-lg bg-[#2f6664] px-3 text-[10px] font-semibold text-white hover:bg-[#255452]"><CheckCircle2 className="h-3.5 w-3.5"/>Sonraki aşama</button>}
      ...
      {cycle.stage === "LOCKED" && <div className="border-t border-slate-100 px-4 py-2 text-[10px] text-amber-700 dark:border-slate-800 dark:text-amber-300">Bu dönem demo yönetişiminde kilitlidir. Yeni puanlama yerine bir sonraki performans dönemi açılmalıdır.</div>}
    </section>
  );
}
