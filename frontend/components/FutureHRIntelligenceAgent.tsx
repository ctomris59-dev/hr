...
export default function FutureHRIntelligenceAgent({ pathname }: { pathname: string }) {
  ...
  const ask = async (raw: string) => {
    ...
    try {
      ...
      const emptyPackage: AgentPackage = {
        ...
        toolsUsed: [], toolResults: [], evidenceSources: [], evidenceGaps: [], preparedActions: [], externalContext: {},
      };
      setConversation((rows) => [...rows, { id: `agent-${Date.now()}`, question: clean, package: emptyPackage, result: { error: error instanceof Error ? error.message : "FutureHR Intelligence servisine ulaşılamadı." } }].slice(-8));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <>
      ...
      {open && (
        <div className="fixed inset-0 z-[100]" role="dialog" aria-modal="true" aria-labelledby="futurehr-agent-title">
          <button type="button" aria-label="FutureHR Intelligence'ı kapat" onClick={() => setOpen(false)} className="absolute inset-0 bg-slate-950/40 backdrop-blur-[1px]" />
          <aside className="absolute inset-y-0 right-0 flex w-full max-w-[680px] flex-col border-l border-slate-200 bg-[#f5f7f8] shadow-[-24px_0_70px_rgba(15,23,42,.12)] dark:border-slate-800 dark:bg-[#0f151b]">
            <header className="border-b border-slate-200 bg-white px-5 py-4 dark:border-slate-800 dark:bg-[#141b22]">
              <div className="flex items-start justify-between gap-4">
                <div className="flex items-start gap-3">
                  <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-[linear-gradient(135deg,#234e4a,#467a75)] text-white shadow-sm"><BrainCircuit className="h-5 w-5" /></span>
                  <div>
                    ...
                    <p className="mt-1 text-[11px] text-slate-500">Uygulamadaki kanıtları tarar, ilgili araçları çalıştırır ve yöneticinin önüne net cevap getirir.</p>
                  </div>
                </div>
            ...
            <div className="min-h-0 flex-1 overflow-y-auto p-5">
              {conversation.length === 0 && !loading && (
                <div className="space-y-4">
                  <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-[#141b22]">
                    <div className="flex items-start gap-3">
                      <Database className="mt-0.5 h-4 w-4 text-[#2f6664]" />
                      <div>
                  ...
                  <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white dark:border-slate-800 dark:bg-[#141b22]">
                    <div className="border-b border-slate-100 px-4 py-3 dark:border-slate-800"><p className="text-[10px] font-bold uppercase tracking-[.1em] text-slate-400">Örnek sorular</p></div>
                    <div className="divide-y divide-slate-100 dark:divide-slate-800">
                      {quick.map((prompt) => (
                        <button key={prompt} type="button" onClick={() => void ask(prompt)} className="flex w-full items-center justify-between gap-3 px-4 py-3 text-left text-xs font-medium text-slate-700 hover:bg-slate-50 dark:text-slate-200 dark:hover:bg-slate-800/60">
                          <span>{prompt}</span><ArrowRight className="h-3.5 w-3.5 shrink-0 text-slate-300" />
                        </button>
                      ))}
                    </div>
                  </section>
              ...
              {loading && (
                <div className="flex min-h-[260px] flex-col items-center justify-center text-center" aria-live="polite">
                  <span className="flex h-11 w-11 items-center justify-center rounded-xl border border-slate-200 bg-white text-[#2f6664] shadow-sm dark:border-slate-700 dark:bg-slate-800"><Loader2 className="h-5 w-5 animate-spin" /></span>
            ...
            <form onSubmit={submit} className="border-t border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-[#141b22]">
              <div className="rounded-2xl border border-slate-200 bg-white p-2 shadow-sm focus-within:border-[#7ba4a1] focus-within:ring-4 focus-within:ring-[#7ba4a1]/10 dark:border-slate-700 dark:bg-slate-900">
                <label className="sr-only" htmlFor="futurehr-agent-question">FutureHR Intelligence sorusu</label>
                <textarea id="futurehr-agent-question" value={question} onChange={(event) => setQuestion(event.target.value)} rows={2} placeholder="Örn. Ayşe Kaya'ya hangi eğitimleri vermeliyiz ve neden?" className="min-h-[58px] w-full resize-none bg-transparent px-2 py-2 text-xs outline-none placeholder:text-slate-400" />
                <div className="flex items-center justify-between gap-3 px-1 pb-1">
                  <div className="flex items-center gap-1.5 text-[9.5px] text-slate-400"><ShieldCheck className="h-3.5 w-3.5" /> Yetki kapsamı dışındaki veri ajan tarafından kullanılmaz.</div>
                  <button type="submit" disabled={loading || !question.trim()} aria-label="Soruyu gönder" className="flex h-9 w-9 items-center justify-center rounded-lg bg-[#2f6664] text-white transition hover:bg-[#285a57] disabled:opacity-40"><Send className="h-4 w-4" /></button>
                </div>
              </div>
            </form>
          </aside>
...
function ConversationCard({ item, currentUserRole, drafted, onOpenRoute, onPrepareAction }: {
  ...
  return (
    <div className="space-y-3">
      <div className="ml-auto max-w-[88%] rounded-2xl rounded-br-md bg-[#173d3a] px-4 py-3 text-xs leading-5 text-white shadow-sm">{item.question}</div>
      {item.result.error ? (
        ...
        <>
          <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-[#141b22]">
            ...
            <p className="mt-3 text-[14px] font-medium leading-6 text-slate-900 dark:text-white">{analysis.answer}</p>
            <p className="mt-2 text-[10.5px] leading-4 text-slate-400">{analysis.confidenceReason}</p>
                ...
                {analysis.recommendations.slice(0, 5).map((recommendation, index) => (
                  <div key={`${recommendation.title}-${index}`} className="p-4">
                    <div className="flex items-start justify-between gap-3">
                      <div><p className="text-xs font-semibold text-slate-900 dark:text-white">{index + 1}. {recommendation.title}</p><p className="mt-1.5 text-[10.5px] leading-5 text-slate-500">{recommendation.why}</p><p className="mt-2 text-[10px] leading-4 text-[#2f6664]">Kanıt: {recommendation.evidence}</p></div>
            ...
            <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-[#141b22]">
              ...
              <div className="mt-3 grid gap-2 sm:grid-cols-2">
                {item.package.evidenceSources.slice(0, 8).map((source) => (
                  <button key={source.id} type="button" disabled={!canAccessRoute(currentUserRole, source.route)} onClick={() => onOpenRoute(source.route, item.package, source.label)} className="rounded-xl border border-slate-200 bg-slate-50/70 p-3 text-left transition hover:border-slate-300 hover:bg-white disabled:cursor-not-allowed disabled:opacity-60 dark:border-slate-700 dark:bg-slate-800/60">
                    <div className="flex items-start justify-between gap-2"><p className="text-[10.5px] font-semibold text-slate-800 dark:text-slate-100">{source.label}</p><ExternalLink className="h-3 w-3 text-slate-300" /></div>
                    <p className="mt-1 text-[9.5px] leading-4 text-slate-500">{source.detail}</p>
                  </button>
                ))}
              </div>
            </section>
            ...
            <section className="rounded-2xl border border-[#cbdad8] bg-[#f1f6f5] p-4 dark:border-[#315f5c] dark:bg-[#172321]">
              ...
              <div className="mt-3 space-y-2">
                {item.package.preparedActions.slice(0, 5).map((action) => {
                  ...
                  return <div key={action.id} className="flex items-start justify-between gap-3 rounded-xl border border-white/70 bg-white/75 p-3 dark:border-slate-700 dark:bg-slate-900/70">
                    <div><p className="text-[10.5px] font-semibold text-slate-800 dark:text-slate-100">{action.label}</p><p className="mt-1 text-[9.5px] leading-4 text-slate-500">{action.description}</p></div>
          ...
          {analysis.evidenceGaps?.length > 0 && (
            <section className="rounded-2xl border border-amber-200 bg-amber-50 p-4 dark:border-amber-900/60 dark:bg-amber-950/20">
              ...
              <div className="mt-2 space-y-1.5">{analysis.evidenceGaps.slice(0, 6).map((gap) => <p key={gap} className="text-[10.5px] leading-4 text-amber-800">• {gap}</p>)}</div>
            </section>
          )}

          <div className="flex items-start gap-2 rounded-xl border border-slate-200 bg-white p-3 text-[10px] leading-4 text-slate-500 dark:border-slate-800 dark:bg-[#141b22]"><ShieldCheck className="mt-0.5 h-3.5 w-3.5 shrink-0 text-[#2f6664]" /><span>{analysis.guardrail}</span></div>
        </>
      ) : null}
    </div>
  );
