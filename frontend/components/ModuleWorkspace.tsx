...
export default function ModuleWorkspace({pathname,children}:{pathname:string;children:ReactNode}){
  ...
  return <div className={`module-workspace workspace-${config.id}`} data-module={config.id} style={style}>
    <section className="module-hero mb-4 overflow-hidden border bg-white dark:border-slate-800 dark:bg-slate-900" aria-label={`${config.title} çalışma akışı`}><div className="grid gap-3 px-4 py-3 md:grid-cols-[minmax(0,1.35fr)_auto] md:items-center"><div className="flex min-w-0 items-center gap-3"><span className="flex h-8 w-8 shrink-0 items-center justify-center text-white" style={{background:config.accent}}><Icon className="h-4 w-4" strokeWidth={1.7}/></span><div className="min-w-0"><p className="text-[9px] font-semibold uppercase tracking-[.12em] text-slate-400">Çalışma akışı</p><p className="mt-0.5 text-[12px] leading-5 text-slate-600 md:truncate dark:text-slate-300" title={config.focus}>{config.focus}</p></div></div><div className="flex flex-wrap gap-1.5">{config.steps.map((step,index)=><span key={step} className="rounded-md border px-2 py-1 text-[10px] font-medium text-slate-600 dark:text-slate-400"><b className="mr-1.5 font-semibold tabular-nums text-slate-400">{String(index+1).padStart(2,"0")}</b>{step}</span>)}</div></div></section>
