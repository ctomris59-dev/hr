...
const menuItems = [
  ...
  { href: "/organizasyon", label: "Organizasyon", icon: Building2, adminOnly: true, section: "Genel" },
  { href: "/degerlendirme", label: "360 Değerlendirme", icon: RefreshCw, section: "Performans & Yetenek" },
  ...
  return (
    <div data-testid="app-shell" className="flex h-screen overflow-hidden bg-[#f5f7fa] dark:bg-slate-950">
      <button
        ...
        className="lg:hidden fixed top-3 left-3 z-50 flex h-9 w-9 items-center justify-center rounded-lg border border-slate-200 bg-white text-slate-700 shadow-sm dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200"
      ...
      <aside
        ...
        className={`${sidebarOpen ? "translate-x-0" : "-translate-x-full"} lg:translate-x-0 fixed lg:static inset-y-0 left-0 z-40 w-[248px] flex-shrink-0 border-r border-slate-800 bg-[#111827] text-slate-300 transition-transform duration-200 ease-out`}
      >
          ...
          <nav className="flex-1 overflow-y-auto px-3 py-4">
            {filteredMenuItems.map((item, index) => {
              ...
              return (
                <div key={item.href}>
                  {showSection && (
                    <div className={`${index === 0 ? "mt-0" : "mt-5"} mb-1.5 px-3 text-[10px] font-semibold uppercase tracking-[0.12em] text-slate-600`}>
                      {item.section}
                    </div>
                  )}
                  <Link
                    ...
                    className={`group relative mb-0.5 flex h-9 items-center gap-3 rounded-lg px-3 text-[13px] font-medium ${
                      isActive
          ...
          <div className="flex-shrink-0 border-t border-white/[0.07] p-3">
            {user && (
              <div className="mb-2 flex items-center gap-3 rounded-lg px-2 py-2">
                <div className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-lg bg-slate-800 text-[11px] font-semibold text-slate-200 ring-1 ring-white/[0.08]">
                  {userInitials}
                </div>
                <div className="min-w-0 flex-1">
                  <p className="truncate text-[12px] font-semibold text-slate-200">{user.name}</p>
                  <p className="truncate text-[10px] text-slate-500">{user.position || user.role}</p>
                </div>
              </div>
            )}
            <button
              onClick={handleLogout}
              className="flex h-9 w-full items-center gap-3 rounded-lg px-3 text-[12px] font-medium text-slate-500 hover:bg-red-500/10 hover:text-red-300"
            >
              <LogOut size={15} />
              <span>Çıkış Yap</span>
            </button>
      ...
      {sidebarOpen && (
        <div className="lg:hidden fixed inset-0 z-30 bg-slate-950/45" onClick={() => setSidebarOpen(false)} />
      )}
      ...
      <div className="flex min-w-0 flex-1 flex-col overflow-hidden">
        <header className="flex h-16 flex-shrink-0 items-center justify-between border-b border-slate-200 bg-white px-5 lg:px-6 dark:border-slate-800 dark:bg-slate-900">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="hidden sm:flex h-9 items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 text-xs font-medium text-slate-500 hover:border-slate-300 hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-400 dark:hover:bg-slate-800"
            >
        ...
        <main className="flex-1 overflow-y-auto px-4 py-5 sm:px-5 lg:px-6 lg:py-6">
          <ModuleWorkspace pathname={pathname}>
            <ModuleInsights pathname={pathname} />
            {children}
          </ModuleWorkspace>
        </main>
      </div>
