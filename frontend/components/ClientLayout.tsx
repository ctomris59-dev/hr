...
export default function ClientLayout({children}:{children:React.ReactNode}){
  ...
  return <div data-testid="app-shell" className="flex h-screen overflow-hidden bg-[#f6f6f3] dark:bg-[#0f151b]"><AIGovernanceCapture/><DemoDataHardeningBridge/><button onClick={()=>setSidebarOpen(!sidebarOpen)} className="fixed left-3 top-3 z-50 flex h-9 w-9 items-center justify-center rounded-lg border border-slate-200 bg-white text-slate-700 lg:hidden dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200" aria-label="Menüyü aç/kapat">{sidebarOpen?<X size={18}/>:<Menu size={18}/>}</button>
    <aside data-testid="app-sidebar" data-tour="sidebar" className={`${sidebarOpen?"translate-x-0":"-translate-x-full"} fixed inset-y-0 left-0 z-40 w-[264px] flex-shrink-0 border-r border-slate-800/90 bg-[#101722] text-slate-300 transition-transform duration-200 ease-out lg:static lg:translate-x-0`}><div className="flex h-[100dvh] flex-col overflow-hidden"><div className="futurehr-sidebar-brand flex h-16 flex-shrink-0 items-center border-b border-white/[0.075] px-[22px]"><Image src="/futurehr-brand-dark.svg" alt="FutureHR" width={220} height={56} className="h-[42px] w-auto object-contain" priority/></div><nav className="futurehr-sidebar-nav min-h-0 flex-1 overflow-y-auto px-3 py-2.5">{menuItems.map((item,index)=>{const Icon=item.icon;const isActive=item.key===activeMenuKey;const showSection=index===0||menuItems[index-1]?.section!==item.section;return <div key={item.key}>{showSection&&<div className={`${index===0?"mt-0":"mt-3"} futurehr-sidebar-section mb-1.5 px-3 text-[10px] font-semibold uppercase leading-4 tracking-[0.16em] text-slate-500`}>{item.section}</div>}<Link data-tour-route={item.href} href={item.href} onClick={()=>setSidebarOpen(false)} className={`futurehr-sidebar-item group relative mb-1 flex min-h-10 items-center gap-3 rounded-lg px-3 py-2 text-[13.5px] leading-5 transition-colors duration-150 ${isActive?"bg-white/[0.09] font-semibold text-white":"font-medium text-slate-300/90 hover:bg-white/[0.05] hover:text-white"}`}>{isActive&&<span className="absolute left-0 h-6 w-[2px] rounded-r-full bg-[#78a9a5]"/>}<span className={`flex h-6 w-6 flex-shrink-0 items-center justify-center rounded-md transition-colors ${isActive?"text-[#9bc1bd]":"text-slate-500 group-hover:text-slate-300"}`}><Icon size={17} strokeWidth={1.7}/></span><span className="truncate tracking-[-0.01em]">{item.label}</span></Link></div>;})}</nav><div className="futurehr-sidebar-footer flex-shrink-0 border-t border-white/[0.075] p-3">{user&&<div className="futurehr-sidebar-user mb-1.5 flex items-center gap-3 rounded-lg px-2 py-2"><div className="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-lg border border-white/[0.09] bg-[#1d2934] text-[11px] font-semibold text-slate-100">{userInitials}</div><div className="min-w-0 flex-1"><p className="truncate text-[12.5px] font-semibold leading-4 text-slate-100">{user.name}</p><p className="futurehr-sidebar-user-role mt-0.5 truncate text-[10.5px] leading-4 text-slate-500">{user.position||user.role}</p></div></div>}<button onClick={handleLogout} className="futurehr-sidebar-logout flex h-9 w-full items-center gap-3 rounded-lg px-3 text-[12.5px] font-medium text-slate-500 transition-colors hover:bg-red-500/10 hover:text-red-300"><LogOut size={15}/><span>Çıkış Yap</span></button></div></div></aside>
    {sidebarOpen&&<div className="fixed inset-0 z-30 bg-slate-950/45 lg:hidden" onClick={()=>setSidebarOpen(false)}/>}<div className="flex min-w-0 flex-1 flex-col overflow-hidden"><header data-tour="topbar" className="flex h-16 flex-shrink-0 items-center justify-between border-b border-[#dfe3e1] bg-[#fbfbf8] px-4 sm:px-5 lg:px-6 dark:border-slate-800 dark:bg-[#141b22]"><WelcomeWidget/><div className="flex min-w-0 items-center gap-1.5 sm:gap-2"><FutureHRCopilot pathname={normalizedPathname}/><DemoPresentationMode/><GuidedOnboarding/><DemoRoleSwitcher/><button type="button" onClick={()=>document.dispatchEvent(new KeyboardEvent("keydown",{key:"k",ctrlKey:true}))} className="hidden h-9 items-center gap-2 rounded-lg border border-slate-200 bg-transparent px-3 text-xs font-medium text-slate-500 hover:bg-slate-100/70 xl:flex dark:border-slate-700 dark:text-slate-400"><SearchIcon className="h-3.5 w-3.5"/><span>Ctrl + K</span></button><ThemeToggle/><NotificationBell/></div></header><main data-tour="workspace" className="flex-1 overflow-y-auto px-3 py-4 sm:px-5 lg:px-7 lg:py-6"><ModuleWorkspace pathname={normalizedPathname}>{children}</ModuleWorkspace></main></div><CommandPalette/>
  </div>;
}
