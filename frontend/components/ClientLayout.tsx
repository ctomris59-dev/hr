...
const menuItems = [
  ...
  { href: "/kalibrasyon", label: "Kalibrasyon Merkezi", icon: Scale, section: "Performans & Yetenek" },
      ...
      <aside data-testid="app-sidebar" data-tour="sidebar" className={`${sidebarOpen ? "translate-x-0" : "-translate-x-full"} fixed inset-y-0 left-0 z-40 w-[248px] flex-shrink-0 border-r border-slate-800 bg-[#101722] text-slate-300 transition-transform duration-200 ease-out lg:static lg:translate-x-0`}>
        <div className="flex h-[100dvh] flex-col overflow-hidden">
          <div className="futurehr-sidebar-brand flex h-16 flex-shrink-0 items-center border-b border-white/[0.07] px-5">
            <Image src="/futurehr-brand-dark.svg" alt="FutureHR" width={220} height={56} className="h-10 w-auto object-contain" priority />
          </div>
          ...
          <nav className="futurehr-sidebar-nav min-h-0 flex-1 overflow-y-auto px-2.5 py-2.5 lg:overflow-hidden">
            {filteredMenuItems.map((item, index) => {
              ...
              return (
                <div key={item.href}>
                  {showSection && <div className={`${index === 0 ? "mt-0" : "mt-3"} futurehr-sidebar-section mb-1 px-2.5 text-[9px] font-semibold uppercase leading-3 tracking-[0.13em] text-slate-600`}>{item.section}</div>}
                  <Link data-tour-route={item.href} href={item.href} onClick={() => setSidebarOpen(false)} className={`futurehr-sidebar-item group relative mb-px flex h-8 items-center gap-2.5 rounded-lg px-2.5 text-[12px] font-medium transition-all ${isActive ? "bg-white/[0.09] text-white shadow-[inset_0_0_0_1px_rgba(255,255,255,.035)]" : "text-slate-400 hover:bg-white/[0.05] hover:text-slate-200"}`}>
                    ...
                    <Icon size={15} strokeWidth={1.8} className={isActive ? "text-indigo-300" : "text-slate-500 transition-colors group-hover:text-slate-300"} />
          ...
          <div className="futurehr-sidebar-footer flex-shrink-0 border-t border-white/[0.07] p-2.5">
            {user && <div className="futurehr-sidebar-user mb-1 flex items-center gap-2.5 rounded-lg px-2 py-1.5">
              <div className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-lg bg-gradient-to-br from-indigo-500/20 to-teal-500/15 text-[10px] font-semibold text-slate-200 ring-1 ring-white/[0.08]">{userInitials}</div>
              <div className="min-w-0 flex-1"><p className="truncate text-[11.5px] font-semibold text-slate-200">{user.name}</p><p className="futurehr-sidebar-user-role truncate text-[9.5px] text-slate-500">{user.position || user.role}</p></div>
            </div>}
      ...
      <div className="flex min-w-0 flex-1 flex-col overflow-hidden">
        <header data-tour="topbar" className="flex h-16 flex-shrink-0 items-center justify-between border-b border-slate-200/90 bg-white px-5 lg:px-6 dark:border-slate-800 dark:bg-slate-900">
          ...
          <div className="flex items-center gap-2">
            <FutureHRCopilot pathname={normalizedPathname} />
            <GuidedOnboarding />
            <DemoRoleSwitcher />
            <button type="button" onClick={() => document.dispatchEvent(new KeyboardEvent("keydown", { key: "k", ctrlKey: true }))} className="hidden h-9 items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 text-xs font-medium text-slate-500 hover:border-slate-300 hover:bg-slate-50 sm:flex dark:border-slate-700 dark:bg-slate-900 dark:text-slate-400 dark:hover:bg-slate-800">
              <SearchIcon className="h-3.5 w-3.5" /><span>Ctrl + K</span>
            </button>
            <ThemeToggle />
            <NotificationBell />
          </div>
