...
export default function NotificationBell() {
  ...
  return <div className="relative">
    <button ref={buttonRef} onClick={() => setOpen((value) => !value)} aria-label="Bildirimler" aria-expanded={open} aria-haspopup="dialog" aria-controls="futurehr-notification-panel" className="relative flex h-9 w-9 items-center justify-center rounded-lg border border-slate-200 bg-white text-slate-600 hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300"><Bell className="h-4 w-4"/>{unreadCount>0&&<span className="absolute -right-1.5 -top-1.5 flex h-4 min-w-4 items-center justify-center rounded-full bg-red-600 px-1 text-[9px] font-bold text-white ring-2 ring-white dark:ring-slate-900" aria-label={`${unreadCount} okunmamış bildirim`}>{unreadCount>9?"9+":unreadCount}</span>}</button>
    {open && createPortal(<div id="futurehr-notification-panel" ref={panelRef} style={style||undefined} role="dialog" aria-label="Bildirimler" className="fixed z-[9999] w-[min(360px,calc(100vw-16px))] overflow-hidden rounded-xl border border-slate-200 bg-white shadow-[0_18px_45px_rgba(15,23,42,.18)] dark:border-slate-700 dark:bg-slate-900">
      <div className="flex items-center justify-between border-b border-slate-100 px-4 py-3 dark:border-slate-800"><div><h3 className="text-sm font-semibold">Bildirimler</h3><p className="mt-0.5 text-[10px] text-slate-400">Gerçek FutureHR iş olaylarından üretilir.</p></div><div className="flex items-center gap-2">{notifications.length>0&&<button type="button" onClick={clearAll} className="text-[10px] font-semibold text-slate-500 hover:text-slate-800">Tümünü okundu say</button>}<button type="button" onClick={()=>{setOpen(false);buttonRef.current?.focus();}} aria-label="Bildirimleri kapat" className="flex h-7 w-7 items-center justify-center rounded-md text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800"><X className="h-4 w-4"/></button></div></div>
      <div className="max-h-[420px] overflow-y-auto">{notifications.length===0?<div className="px-4 py-8 text-center"><Bell className="mx-auto h-8 w-8 text-slate-200"/><p className="mt-2 text-xs text-slate-400">Şu anda aksiyon bekleyen bildirim yok.</p></div>:<div className="divide-y divide-slate-100 dark:divide-slate-800">{notifications.map((item)=><button key={item.id} type="button" onClick={()=>activate(item)} aria-label={item.link?`${item.message} — aç`:item.message} className={`flex w-full items-start gap-3 px-4 py-3 text-left hover:bg-slate-50 focus-visible:bg-slate-50 dark:hover:bg-slate-800/50 dark:focus-visible:bg-slate-800/50 ${!item.read?"bg-indigo-50/40 dark:bg-indigo-950/10":""}`}><span className="mt-0.5"><Icon type={item.type}/></span><span className="min-w-0 flex-1"><span className={`block text-xs leading-5 ${!item.read?"font-semibold text-slate-900 dark:text-white":"text-slate-600 dark:text-slate-300"}`}>{item.message}</span><span className="mt-1 flex items-center gap-2 text-[9px] text-slate-400"><span>{relative(item.timestamp)}</span>{item.source&&<span>· {item.source}</span>}{item.link&&<span className="font-semibold text-indigo-500">Aç →</span>}</span></span>{!item.read&&<span className="mt-2 h-2 w-2 rounded-full bg-indigo-500" aria-hidden="true"/>}</button>)}</div>}</div>
    </div>, document.body)}
  </div>;
}
